...
---
  ...
  ## 🔷 Strict Mode: Only Explicit page_ids (v4.1)

  - Обробляються лише `request.page_ids` (після dedupe).
  - Whitelist — **фільтр**, а не джерело сторінок (entry points без рекурсії).
  - Заборонено: get_child_pages, get_descendants, get_ancestors, get_space, get_related_pages, get_incoming_links.
  - Дозволені Confluence виклики в tag-pages: get_page(page_id, expand="body.storage"), get_labels(page_id).
  - Контекст для AI: очищений HTML → текст, максимум 3000 символів; логуються метрики скорочення.
  - Однакова поведінка щодо джерела сторінок у TEST / SAFE_TEST / PROD.

  ### Приклад strict запиту
  ```bash
  curl -X POST http://localhost:8000/bulk/tag-pages \
    -H "Content-Type: application/json" \
    -d '{
      "space_key": "nkfedba",
      "page_ids": ["19699862097", "19727221584"],
      "dry_run": true
    }'
  ```

  ### Очікування
  - Confluence: 2× get_page(body.storage) + 2× get_labels (для 2 ID).
  - AI: 2 виклики з контекстом ≤3000 символів кожний.
  - Жодних додаткових сторінок або обходів дерева.

  ## Centralized AI Context (v4.1)
  - HTML очищення (scripts/style/iframe/macros) → текст.
  - Ліміт контексту: TAGGING_MAX_CONTEXT_CHARS (з .env, дефолт 3000).
  - Єдина функція `prepare_ai_context()` використовується для tag-pages.
  - Логіка обходу/whitelist не змінена, лише підготовка контенту для AI.
...
### v2.1 (2025-12-29)
- ✅ Whitelist інтеграція (фільтрація за дозволеними сторінками)
- ✅ Режимна логіка (TEST, SAFE_TEST, PROD)
- ✅ Детальне логування
- ✅ Помилки з описом рішень
- ✅ Порівняння з іншими ендпоінтами
- ✅ Best practices та security рекомендації
- ✅ Troubleshooting секція
- ✅ Приклади використання для всіх режимів

### v2.0 (2025-12-20)
- ✅ Базова функціональність tag-pages
- ✅ JSON request/response формати
- ✅ Dry-run режим
- ✅ Whitelist перевірка
